...
import { declineInvitation } from "@/server/repositories/invitationRepository"
...
/** `POST /api/invitations/:invitationId/decline` — only the invited user may decline their own invitation. */
export async function POST(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    ...
    const invitation = await declineInvitation(invitationId, user.id)
