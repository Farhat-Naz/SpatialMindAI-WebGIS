...
import { transferOwnership } from "@/server/repositories/membershipRepository"
import { transferOwnershipSchema } from "@/shared/contracts/membership.schema"
...
/** `POST /api/projects/:projectId/transfer-ownership` — Owner-only (US1 scenario 6). */
export async function POST(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    ...
    await transferOwnership(projectId, user.id, parsed.data.newOwnerUserId)
    return respond(request, startedAt, 200, { success: true })
  } catch (error) {
